...
Two-phase extraction is enabled for all 5 brew methods (`supportsTwoPhase: true` for all in MethodConfig). V60 uses the real multi-step pour schedule; all other methods use a synthetic single-pour schedule generated by `generateSyntheticSchedule()`. Per-method extraction rates are controlled by `methodModifierFast` and `methodModifierSlow` in the MethodConfig registry. See [Per-Method Behavior](./per-method-behavior.md) for the full modifier table and physical rationale.
